...
#ifdef EC_RTDM
    if (!(master->task = (rtdm_task_t *) kmalloc(sizeof(rtdm_task_t), GFP_KERNEL))) {
        EC_MASTER_ERR(master, "Error allocating rtdm_task memory~\n");
        return ERR_PTR(-ENOMEM);
    }

    ret = rtdm_task_init(master->task, name, thread_func, master, 96, 0);
    if (ret != 0) {
        kfree(master->task);
        master->task = NULL;
...
/** Starts the master thread without RTDM.
 *
 * \retval  0 Success.
 * \retval <0 Error code.m
 */
int ec_master_nrthread_start(
        ec_master_t *master, /**< EtherCAT master */
        int (*thread_func)(void *), /**< thread function to start */
        const char *name /**< Thread name. */
        )
...
/** Stops the master thread without rt.
 */
void ec_master_nrthread_stop(
        ec_master_t *master /**< EtherCAT master */
        )
{
    unsigned long sleep_jiffies;
...
#endif
    ec_master_nrthread_stop(master);
...
#endif
    if (ec_master_nrthread_start(master, ec_master_idle_thread,
                "EtherCAT-IDLE")) {
